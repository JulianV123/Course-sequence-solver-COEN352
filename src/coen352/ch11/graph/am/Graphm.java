...
/**
 * Graphm.java was given to us on moodle
 */
...
class Graphm implements Graph { // Graph: Adjacency matrix
  ...
  public Integer[] getFanIn(int v) {
	 ...
	 if(size != 0) {
		 ...
		 for(int i=0;i<fanInArray.length();i++) {
		 	 fanInArray.moveToPos(i);
			 array[i] = Integer.valueOf(fanInArray.getValue());
		 }
		 return array;
	 }
	 else
  ...
  public Integer[] getParentNodes(int v) {
      List<Integer> array = new ArrayList<>();

      array = getParentNodesHelper(v, array);

      Set<Integer> noDuplicates = new HashSet(array);
      array.clear();
      array.addAll(noDuplicates);

      int size = array.size();
      Integer[] arrayIntegers = new Integer[size];

      for (int i = 0; i < array.size(); i++) {
          arrayIntegers[i] = array.get(i);
      }

      return arrayIntegers;
  }

  public List<Integer> getParentNodesHelper(int v, List<Integer> parents) {

      Integer[] array = getFanIn(v);

      if (array != null) {
          for (Integer number : array) {
              parents.add(number);
              getParentNodesHelper(number, parents);
          }
